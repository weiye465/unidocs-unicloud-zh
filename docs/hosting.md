...
如果你已经有备案过的域名，直接解析过来即可；如果你要新注册域名，首先自行在网上购买，然后注意域名如果想在国内正常绑定阿里云或腾讯云，需要域名备案。
这里的备案流程和传统云主机略有不同，涉及uniCloud没有固定ip的问题。
1. 推荐使用支付宝云，会赠送备案码，[详见](https://doc.dcloud.net.cn/uniCloud/price.html#%E5%A4%87%E6%A1%88%E7%A0%81)
2. 可以去买花生壳的备案服务。这里有开发者分享的经验贴：[https://ask.dcloud.net.cn/article/38116](https://ask.dcloud.net.cn/article/38116)；
3. 也可以临时买一个短期传统云，走传统备案
...
- 全路径文件：指定完整的文件路径进行缓存过期时间设置，格式为/index.html，支持完整路径加文件类型匹配模式，如/test/*.jpg。仅腾讯云支持。
...
阿里云前端网页托管在绑定自定义域名后，支持配置CDN安全策略：[https://uniapp.dcloud.net.cn/uniCloud/cdn-security-policy.html](storage/cdn-security-policy.md)
