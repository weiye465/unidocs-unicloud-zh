...
登录[uniAd控制台](https://uniad.dcloud.net.cn)，在右上角点击 `uni-ad-admin` 按钮，在弹出框中选择已部署含`uni-ad-admin`插件的`uni-admin`项目空间：
...
如果你的原服务空间过期或其它原因需要变更收益数据的服务空间，则需再次登录[uniAd控制台](https://uniad.dcloud.net.cn)，点击右上角 `uni-ad-admin` 按钮，在弹出对话框中选择新的服务空间即可。
