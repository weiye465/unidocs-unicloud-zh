...
申请迁移需要加群：[扩展存储技术交流群](https://im.dcloud.net.cn/#/?joinGroup=65436862cc41b0763842cfc9)
...
申请方式：在群里发送内容，我想申请七牛云官方账号存储空间迁移到扩展存储

[七牛云跨区域迁移教程](https://developer.qiniu.com/kodo/8557/set-the-cross-regional-synchronization)

#### 其他OSS迁移到扩展存储@q5-2

**实际场景**

我用的是阿里云OSS（腾讯云OSS或其他OSS也可以），我想将Ta迁移到扩展存储，应该如何迁移?

**迁移方案一**

将原先的OSS内的文件全部下载到你的电脑上，然后通过 [本地电脑文件快速批量上传到扩展存储](#q5-3) 的方式上传到扩展存储

**迁移方案二**

可以申请将扩展存储的回源地址修改为你原先OSS的源站地址，这样当用户访问文件时，扩展存储如果未找到文件会尝试从你原先的源站进行获取并保存，以此实现动态迁移，此方案需要一段时间内保留原先oss内的文件，直到扩展存储几乎全部回源后可以释放原先oss（注意：如果某些文件一直没访问，那么扩展存储里不会有这些文件）

申请方式：在群里发送内容，我想申请阿里云OSS迁移到扩展存储

**迁移方案三**

只将uni-cdn的回源地址修改为你原先OSS的源站地址，这样只使用了uni-cdn，扩展存储内使不存储文件的，文件全在你原先的OSS中

申请方式：在群里发送内容，我想申请uni-cdn回源到阿里云OSS

**特别注意：方案三只有CDN消耗大户才能申请**

#### 本地电脑文件快速批量上传到扩展存储@q5-3

可使用七牛云提供的 `qshell` 工具进行操作
