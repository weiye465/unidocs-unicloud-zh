...
- 使用HBuilderX 3.6.10-Alpha或3.6.5-正式版之前的版本发布的应用，迁移后上传文件会报错（错误信息为：`文件上传失败`，web端上传请求会返回403错误码），需要使用更新的HBuilderX版本重新发布应用（不管是app、小程序、web均需要重新发布，app端可使用wgt更新）。cli创建的项目需要更新项目依赖，参考：[修改依赖为指定版本](https://uniapp.dcloud.net.cn/quickstart-cli.html#cliversion)
