...
新增页面可以自己开发页面，也可以从插件市场下载插件。页面如需添加菜单，参见上文的[菜单管理](#menus)。
...
1.执行`npm config set registry https://registry.npm.taobao.org/`配置淘宝镜像源
2.进入到相应的uni-id-co云对象目录，在这个文件夹里面打开终端，并执行`npm install`手动安装依赖，后右键该云对象文件夹并选择上传部署。
