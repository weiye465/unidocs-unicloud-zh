...
uni一键登录的费用分为2个部分：
- 运营商的费用，0.02元/次，失败不计费
- 依赖的uniCloud费用，每次约0.0000139元，10万次调用才1.39元，几乎可忽略不计，详见下方uniCloud费用说明；

### uniCloud费用说明

uni一键登录在实际使用中需要依赖`uniCloud`云服务，这方面费用如下：在使用阿里云正式版后，每次一键登陆请求大约需要消耗uniCloud费用0.0000139元，几乎可以忽略不计。也可以粗略认为每次使用一键登陆的成本为0.0200139元/次。费用计算规则如下：
