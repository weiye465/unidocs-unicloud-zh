...
**一般产生慢查询日志都是因为数据表未配置索引，请根据日志内容中的查询条件配置对应索引**。详见[数据库索引相关介绍](./db-index.md)

::: warning 注意事项
阿里云服务空间当每分钟新产生的慢查询日志超过200条或文档扫描数超2000万行时会触发阿里云限流：Throttling.Api: Request was denied due to api flow control. RequestId，导致服务异常。请及时对慢查询进行优化，如合理配置查询索引、优化表结构及业务代码查询条件等。支付宝云无此限制，推荐使用支付宝云。
:::
