...
开通要求：前一日账号下所有服务空间产生的CDN流量大于1MB，CDN流量包含云存储及绑定自定义域名的前端网页托管。
...
### 支付宝云
支付宝云没有开通前条件限制，只需前往[uniCloud控制台](https://unicloud.dcloud.net.cn "uniCloud控制台") --> 服务空间详情 --> 安全 --> CDN安全策略页面就可以配置访问规则、查看访问报表。访问报表有一天延迟。
