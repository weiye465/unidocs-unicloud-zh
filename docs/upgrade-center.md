...
每个App开发者都要开发升级功能，这是巨大的社会资源浪费。DCloud推出 uni-upgrade-center，让应用开发更轻松、高效，让开发者专注于自己的业务。
...
- 发布原生App安装包
	...
	<div align="center" >
	<img src="https://web-ext-storage.dcloud.net.cn/unicloud/publish_apk.jpg" width="600"></img>
	</div>
...
- 发布wgt资源包
	...
	<div align="center">
	<img src="https://web-ext-storage.dcloud.net.cn/unicloud/publish_wgt.jpg" width="400"></img>
	</div>
...
5. 修改 pages.json
	...
	- 如果是uni-app，需在`pages.json`中添加页面路径。**注：请不要设置为pages.json中第一项**。(在 uni-app 上，为了盖住 tabbar、导航栏以及 vue 页面上的原生元素，使用了背景透明的独立页面)
	...
	```
	- 如果是 uni-app-x 项目，自 `0.9.0` 起，使用 `dialogPage` 实现弹窗，需要在 `pages.json` 中添加页面路径
...
> 扩展存储查看：[文档](https://doc.dcloud.net.cn/uniCloud/ext-storage/service.html)
...
1. 使用升级中心的用户需要什么权限？答：`admin` 角色或 `READ_OPENDB_APP_VERSIONS`、`CREATE_OPENDB_APP_VERSIONS`、`UPDATE_OPENDB_APP_VERSIONS`、`DELETE_OPENDB_APP_VERSIONS`、`READ_OPENDB_APP_LIST`、`CREATE_OPENDB_APP_LIST`、`UPDATE_OPENDB_APP_LIST` 权限
2. 在测试时，如果使用标准基座运行，则无法检测出 App 更新：原因是因为标准基座获取到的 appId 是固定的，而不是 manifest 中配置的。建议使用自定义基座测试
