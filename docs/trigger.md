...
|           腾讯云示例			|           阿里云示例			|       支付宝云示例				|          说明																												|
|-------------------------|-------------------------|-------------------------|-------------------------																						|
| * * * * * * *						| 不支持										| * * * * * *							| 每1秒触发一次（阿里云不支持，阿里云最短1分钟触发一次）										|
| */5 * * * * * *					| 不支持										| */5 * * * *							| 每5秒触发一次（阿里云不支持，阿里云最短1分钟触发一次）										|
| 0 * * * * * *						| 0 * * * * *							| 0 * * * * *							| 每1分钟触发一次																												|
| 0 */10 * * * * *				| 0 */10 * * * *					| 0 */10 * * * *					| 每10分钟触发一次																											|
| 0 0 * * * * *						| 0 0 * * * *							| 0 0 * * * *							| 每1小时触发一次（整点触发）																						|
| 0 20 * * * * *					| 0 20 * * * *						| 0 20 * * * *						| 每1小时触发一次（每小时的20分触发，如08:20:00、09:20:00）								|
| 0 0 */2 * * * *					| 0 0 */2 * * *						| 0 0 */2 * * *						| 每2小时触发一次（整点触发）																						|
| 0 0 18 * * * *					| 0 0 18 * * *						| 0 0 18 * * *						| 每天的下午6点触发一次（整点触发，18:00:00）															|
| 0 0 10,14,16 * * * *		| 0 0 10,14,16 * * *			| 0 0 10,14,16 * * *			| 每天上午10点，下午2点，4点触发（整点触发，10:00:00、14:00:00、16:00:00）	|
| 0 */30 9-17 * * * *			| 0 */30 9-17 * * *				| 0 */30 9-17 * * *				| 每天上午9点到下午5点内每半小时触发																			|
| 0 0 2 1 * * *						| 0 0 2 1 * *							| 0 0 2 1 * *							| 每月的1日的凌晨2点触发																									|
| 0 15 10 * * 1-5 *				| 0 15 10 * * 1-5					| 0 15 10 * * 1-5					| 周一到周五每天上午10:15触发																						|
